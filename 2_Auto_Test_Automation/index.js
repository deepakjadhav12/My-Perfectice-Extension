...
function changeAttemptData() {
  ...
  if (!attemptTime || !testData) return;
...
async function submitFeedback() {
  await delay(700);
  localStorage.removeItem("isMainRunning");
...
function stopAll() {
  ...
  removeLocalStorageData("isMainRunning");
  window.location.href = "https://lpu.myperfectice.com/student/home";
}
async function main() {
  localStorage.setItem("isMainRunning", true);
  await delay(3000);
...
chrome.runtime.onMessage.addListener(async (message, sender, sendResponse) => {
  if (message.action === "start") {
    ...
    const isMainRunning = localStorage.getItem("isMainRunning");
    if (!isMainRunning) {
      main();
    }
    startMonitoringLocalStorageChanges();
